...
file_name_data = [f"total_data_backinsulated_{x}_1Hz.pickle" for x in 
             ["immediatecorrection", "1scorrection", "5scorrection"]]

for correction_iteration, correction_type in enumerate(file_name_data):
    
    # debugging 
    if correction_iteration in []:
        continue

    # import 1Hz data
    with open(correction_type, 'rb') as handle:
    ...
    for bc_surface in all_data:
        ...
        axis[0,0].set_ylim([-3, 30])
        axis[0,0].set_yticks(np.linspace(0,30,4))
        axis[0,1].set_ylim([-1.5, 15])
        axis[0,1].set_yticks(np.linspace(0,15,4))
        for ax in axis.flatten()[-2:]:
