...
def plot_numericalexperiments(total_data_1Hz, scenario, scenario_number):
    ...
    for bc_surface in all_data:
        ...
        # formatting the plot
        fig, axis = plt.subplots(2,2, constrained_layout = True, figsize = figure_size, sharex = True)
        ...
        for alpha in alphas:
            ...
            for key in all_data[bc_surface]["incident_heatflux"][alpha]:
                if key > 900:
                    break
                incident_heatflux[alpha][key] = all_data[bc_surface]["incident_heatflux"][alpha][key]/1000
                net_heatflux[alpha][key] = all_data[bc_surface]["net_heatflux"][alpha][key]/1000
                surface_temperature[alpha][key] = all_data[bc_surface]["surface_temperature"][alpha][key] - 288
                error[alpha][key] = all_data[bc_surface]["error"][alpha][key]*100/5000
        ...
        for ax_number, ax in enumerate(axis.flatten()):
            for alpha_number, alpha in enumerate(alphas):
                ax.plot(t_grid_plotting,data_plotting[ax_number][alpha], linewidth = line_width, color = line_color[alpha_number],
                        linestyle = line_styles[alpha_number])
